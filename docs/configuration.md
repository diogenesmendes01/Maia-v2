...
| Variável | Tipo | Default | Segredo | Serviços | Restart | Descrição |
|---|---|---|---|---|---|---|
| `MAIA_HERMES_ENABLED` | string | `false` | não | `runtime` | sim | Integração Hermes (spec §9.3). Com true, o processo HTTP registra o gateway de inferência interno; com false, nada Hermes entra no servidor. Desligar não apaga ledger nem fences. |
| `MAIA_HERMES_INFERENCE_ALLOWED_SOURCES` | string | — | não | `runtime` | sim | Origens, além da loopback, que podem chamar o gateway de inferência Hermes: IPv4/CIDR ou IPv6 exato, separados por vírgula (ex.: a rede do container worker). Vazio = só loopback. Nunca a rede do proxy da borda. |
