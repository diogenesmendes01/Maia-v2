...
| `MAIA_HERMES_KILL_SWITCH` | string | `false` | não | `runtime` | sim | Kill switch do Hermes (K-15): true força maia_react em todo turno novo, por cima de agent_engine_policies. Só desliga; nunca liga o Hermes — ligar é linha por (tenant, agente, canal). Não muda turno já pinado. |
