...
| `maia_turn_claim_total{result="conversation_human_control"}` | A conversa está sob **controle humano** (`conversation_controls.mode` em `pausing` ou `human`): um operador a assumiu e o claim foi recusado. **Não é incidente** — é o hold do P04 funcionando, e cada ponto é uma mensagem que a plataforma deixou de responder automaticamente de propósito. | Nada a fazer no turno: ele fica retido e a automação volta pelo comando `resume`, nunca por tempo (não há auto-resume por timeout ou logout). Se a série crescer **sem** pausa correspondente no console, procure controle órfão: `SELECT id, mode, owner_app_user_id, paused_at FROM conversation_controls WHERE mode <> 'bot'`. |
...
| `maia_stream_blocked_total{reason}` | `/metrics` | `not_head` é ROTINA: cada mensagem que chega enquanto a anterior roda conta um ponto. O que se vigia é a FORMA — `not_head` crescendo sem `maia_turn_claim_total{result="acquired"}` correspondente é uma conversa que parou. `stream_blocked` sustentado é o outbox. `conversation_human_control` (P04.6) é o único motivo BENIGNO da lista: um humano assumiu a conversa, e a série subir junto com as pausas do console é o esperado. |
| `maia_turn_claim_total{result}` | `/metrics` | ganhou `not_head` e `stream_blocked`; depois `stream_poisoned` (#629) e `conversation_human_control` (P04.6, §6.2). `stream_busy` deve CAIR quase a zero depois desta fatia (§11.1). |
