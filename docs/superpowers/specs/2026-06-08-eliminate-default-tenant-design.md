...
`p8e-seed-policies.ts` runs its seed writes under `runWithTenantContext(PRIMARY_CONTEXT, …)` instead of the `default` literal. `setup.ts` needs TWO distinct fixes (a wrap alone is insufficient): (a) wrap the **guarded repo** writes in `main()` (`pessoasRepo.create`, `entidadesRepo.create`, `permissoesRepo.create`, `contasRepo.create`, co-owner block) in `runWithTenantContext(PRIMARY_CONTEXT, …)` — they read tenant from the ALS; (b) the **raw** `db.insert(self_state)` at `setup.ts:26` bypasses the ALS guard, so the wrap does NOT help it — add explicit `tenant_id: PRIMARY_TENANT_ID, agent_id: PRIMARY_AGENT_ID` to its `.values()` (after §3.3 an omitted tenant fails NOT NULL).

### 4.5 Baileys ingress (verify-only — corrected)
Contrary to a stale `core.ts:328` comment, baileys resolves the scope FIRST (`baileys.ts:273-277` → `resolveTenantCtxForUpsert` → `jid-tenant-resolver` → the catch-all repo) and persists INSIDE `runWithTenantContext(ctx.scope, …)`; `createInbound` (`repositories.ts:885`, input type omits tenant/agent) stamps from the ALS. So once §4.1 re-points the catch-all to `primary`, ingress stamps `primary` automatically — **no production change here**, only the unit-test expectations flip. ⚠️ Do NOT wrap the upsert handler in `runWithTenantContext(PRIMARY_CONTEXT)`: that clobbers the resolved per-tenant `ctx.scope` and forces a future tenant-B inbound under `primary` (isolation violation). Verify `jid-tenant-resolver` carries no independent `'default'` literal.
