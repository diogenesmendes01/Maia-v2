...
**Status:** Draft v4 — incorpora a 3ª rodada do review de design. Mudanças vs. v3: §1.6 CHECK de pareamento NULL (`agent_id`/`proposal_source` ambos nulos ou ambos preenchidos) + partial unique só quando ambos não-nulos (alta — NULLs são distintos em Postgres e permitiriam duplicatas), §1.2 mapa de compatibilidade com os LITERAIS reais do schema (`v3.1.1-2026-05-15`) + parser de versões conhecidas para o predecessor legado não falhar na validação antes do mapa (média).
...
- **Mapa de compatibilidade de versões aditivas** (review v3 média, corrigido no v4 média): sem ele, o bump canônico tornaria a PRIMEIRA mudança pós-upgrade de TODO agente `high` (dual), independentemente do campo alterado. Correções v4: (a) o mapa usa os **literais REAIS persistidos** — o valor corrente é `'v3.1.1-2026-05-15'` (`schema.ts` ~2285), não `'v3.1.1'`; o bump introduz p.ex. `'v3.1.2-<data>'` e `SCHEMA_COMPAT = { 'v3.1.2-<data>': ['v3.1.1-2026-05-15'] }`; (b) novo tipo/parser `KnownProfileSchemaVersion` — a validação de corpo aceita QUALQUER versão conhecida (não apenas o literal corrente, que faria o predecessor legado FALHAR na validação antes de chegar ao mapa); corpo em versão desconhecida ⇒ `high` (fail-up), nunca erro de parse. Par no mapa ⇒ a diferença de versão não pesa no risco (só os campos); par fora ⇒ `high`. O mapa vive ao lado do walker e é atualizado a cada bump.
...
- **Substituição da unique global** (índices/constraints não são dados — recriá-los respeita o espírito append-only; documentado no header da migração):
  ...
  - **CHECK de pareamento NULL** (review v4 — em Postgres NULLs são distintos entre si; com `agent_id` nullable dentro da chave, rows com source preenchido e agent ausente duplicariam indefinidamente): `CHECK ((agent_id IS NULL) = (proposal_source IS NULL))` — ou ambos nulos (legado) ou ambos preenchidos (novo);
  - Partial unique NOVA, escopada e válida apenas quando o escopo é completo: `(tenant_id, agent_id, proposal_source, proposal_id, approver_user_id, decision) WHERE agent_id IS NOT NULL AND proposal_source IS NOT NULL`;
  ...
  - O guard de escrita na camada repo (decisão sem escopo ⇒ erro) permanece como defesa adicional — o CHECK garante que nada escapa dele.
