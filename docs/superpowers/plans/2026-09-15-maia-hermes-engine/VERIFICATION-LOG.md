...
- Nenhuma branch remota nem PR: `git ls-remote --heads origin` (374 heads) filtrado por
  `hermes|mh-|integracao` vazio; `gh pr list --state all` com `--search hermes` e com `--head` de
  cada uma das quatro branches, vazio. (Sem os filtros os dois comandos devolvem resultados — a
  redação anterior desta linha omitia isso; corrigida por apontamento da revisão.)
...
novo em falha**. As 3 falhas a menos que a rodada anterior são `audit-exceptions` (executa o npm
real no Windows) e dois casos de `check-node`. O que está MEDIDO é só isto: o C27 não toca nada que
esses specs leem, e nenhuma falha nova apareceu. Que a causa seja a invocação (Git Bash ×
PowerShell) é **INFERÊNCIA** — não rodei os dois shells sobre a mesma árvore para prová-lo.

⚠️ **Errata da mensagem de `afde96d7`**, que não reescrevo: ela diz "quatro specs TS"; são três
unitários (`agent-engine-contract`, `hermes-wire-contract`, `maia-engine`). Os 15 arquivos
renomeados estão certos.

⚠️ **A afirmação deste V-048 de que a guarda olhava "todos os nomes nas posições que o protocolo
carrega" foi REFUTADA pela revisão adversarial** — ver V-049, onde a guarda é reforçada.
...
### V-049 · Revisão adversarial da integração local, e o que ela derrubou

**Como.** Quatro revisores independentes, somente-leitura, cada um com acesso à spec, ao código, ao
histórico e aos documentos — não ao meu resumo —, instruídos a REFUTAR uma frente: equivalência
da reconstrução, C27, documentação × evidência, e gates reexecutados. Cada afirmação consequente
de um revisor foi conferida por mim antes de virar correção.

**Vereditos:** equivalência **SUSTENTADA** (37 pares por `cat-file` byte a byte, pais traduzidos
pelo mapa, patch-id e texto do patch iguais, e — o teste mais forte — `merge-tree` das tips
ORIGINAIS encadeado dá as mesmas árvores de `9a22fc21`, `ed0cd748` e `3b7fe541`). C27, docs e
gates **PARCIAIS**, nenhum achado bloqueante.

**Achados que viraram correção:**

| # | Achado | Conferido por mim | Correção |
|---|---|---|---|
| 1 | O ledger append-only `migrations/RESERVATIONS.md` reescrito por CRLF (235 linhas) desde `6517579d`; o gate normaliza e não vê | CR 0 na `main`, 235 na ponta; diff `238/235` | `a5730a0f`: diff contra a `main` `+3/−0`; reservas 148/148; specs de ledger 85/85; varredura dos 95 arquivos alterados: 0 com CR. C63 |
| 2 | A guarda do C27 não lê `progress.tool_name`, a quarta chave de nome de tool do wire | `protocol.ts:147` | `95aed007`: extração cobre as quatro chaves, provado com frames sintéticos; vermelho medido com as duas entradas faltantes |
| 3 | Tirar a leitura das linhas cruas não derrubava caso nenhum | reproduzido pela mutação G9 | caso 1 exige origem `raw_line_cases`; G9 agora morre |
| 4 | O caso 3 dizia mais do que faz | leitura do spec | título e comentário dizem que ele só procura o LITERAL antigo |
| 5 | O documento C57 prometia registro do gate na tip final que não existia | grep | seção 7 corrigida; execuções abaixo |
| 6 | Rascunho da PR omitia `AGENTS.md`, `ARCHITECTURE.md`, `docs/architecture/modules/integrations.md`, `src/agent/react-loop.ts` e `src/db/schema.ts`, e subestimava o risco de caminho vivo | `git diff --name-status main` | rascunho reescrito |
| 7 | Rascunho usava `test:leak` "idêntico à linha de base" como evidência de isolamento, sem dizer as 8 falhas nem que uma é asserção real sem controle na `main`; `npm test` sem as 44 falhas | V-027; rodada atual | rascunho declara as contagens e a falta de controle |
| 8 | Matriz P05–P07 dizia "NÃO integrada" no início da célula e "integrado" no fim; §5 do estado obsoleta; filtros do `ls-remote`/`gh` omitidos; "atribuídas teste a teste" sem medição; SHAs antigos "alcançáveis" só localmente; notação `378999e1..8003ee05` dá 19; errata "quatro specs TS" | cada um conferido | corrigidos nos documentos |

**Achados registrados, não corrigidos:** leituras de K-19 além das do C59 (espaço, `MCP:`, homóglifo,
toolset `maia_bridge_v1`) — acrescentadas ao C59 para decisão; pontos cegos do próprio gate de
trailers (`--no-merges`, exige `<email>`, não olha `Signed-off-by`), sem efeito nesta linha
(mensagens dos 5 merges conferidas à parte pelo revisor) — risco residual do guard, não desta
integração; a mensagem de `155d0902` ainda diz "7" por ser cópia fiel.

**Gate de trailers nas tips posteriores** (evento real, sem BOM, `base = main`):
...
A execução na tip do commit que grava este registro não pode estar nele; vai no relatório ao dono.

**Mutação da guarda, rodada 2: 10 medidas, 10 mortas, 0 sobreviventes, 0 puladas** (as sete da
rodada 1 e G8 sem `progress.tool_name`, G9 sem linhas cruas — antes sobrevivente —, G10 com `name`
lido de qualquer frame).

**Suíte unitária em `95aed007`:** `44 | 10556 | 1293 (11893)` — exatamente o previsto antes de
medir (+1 caso da guarda), conjunto de testes em falha IDÊNTICO ao de `afde96d7`.

### V-050 · PR #766 — o primeiro CI sobre este código, e as quatro causas das reprovações

**Contexto.** Com autorização do dono, a linha integrada foi publicada como PR #766 (push de branch
nova, sem force). É a primeira vez que o CI roda sobre a épica. Na tip `f4c78514`, sete jobs
passaram e cinco reprovaram. Cada reprovação foi diagnosticada por investigador somente-leitura,
reproduzida por mim quando a máquina permitia, e corrigida sem afrouxar gate.

| Job | Causa conferida | Correção | Vermelho medido antes |
|---|---|---|---|
| `integration` (node 22.18 e 22.22) | `turn-stream-fairness-real-db` lê um snapshot CROSS-TENANT, e a spec de resume deixava 4 turnos vivos com stream (casos 16–19) — `expected '6' to be '2'` | `a11dda7c`: a spec de resume limpa os próprios turnos (beforeAll, afterEach, afterAll, na ordem das FKs RESTRICT); a expectativa da fairness NÃO mudou | local: fairness `expected 5 to be 1` com exatamente 4 turnos vivos de `resume-tenant`; casos 22–24 de higiene com 8 e 10 turnos vivos |
| `round-trip do drizzle-kit` | dois defaults `bigint(mode:'bigint').default(0n)`; o drizzle-kit 0.31.10 falha ao serializar `0n`, engole a exceção e sai com 0, e a sonda não mostra a saída (C67) | `ff00bb05`: `.default(sql\`0\`)` preservando `mode:'bigint'`; migration 140 já tinha `DEFAULT 0` | `schema-defaults-serializable.spec.ts`: exatamente as duas colunas; sonda real local gerou o up (o down não roda no Windows — caminho com barra invertida) |
| `gitleaks` | 12 achados `generic-api-key`, todos o mesmo UUID sintético de `request_key` nas fixtures do contrato; a 8.24.3 ignora `[[allowlists]]` de topo (C66) | `a0d009a5`: allowlist DE REGRA com `condition = "AND"` (7 caminhos E o valor exato) | spec nova: 15 ocorrências sem cobertura; mutação 5/5 |
| `fault injection (#510)` | `150 executado(s), 0 falha(s), 6 pulado(s)` contra teto 0: os 6 eram o spike do Hermes, que pula sem `MAIA_HERMES_*` (C58) | `1aee0da5`: spike para `tests/hermes-spike/`; o contrato da lane recusa guarda de skip com variável que o job não define | o caso novo acusou exatamente o spike e as duas variáveis |

**Resultado no CI, tip `1aee0da5`:** os 12 jobs **SUCCESS** — `typecheck + test + lint + build`
(22.18 e 22.22), `integration` (22.18 e 22.22), `fault injection (#510)`, `gitleaks`,
`round-trip do drizzle-kit`, `reservation ledger guard`, `npm audit`, `smoke do job migrate`,
`regras de alerta (promtool)`, `build + e2e do console`. É o CI que confirma o `down` do round-trip,
que não rodou aqui. Lido por `gh pr view 766 --json headRefOid,statusCheckRollup`.

**Suíte unitária local em `1aee0da5`:** `44 | 10568 | 1296 (11908)`, conjunto em falha idêntico ao
da linha de base. **Gate de trailers com evento real:** `64 de 69`, exit 0.

### V-051 · PR #766 — os cinco achados de código da revisão do dono

**Método, igual para os cinco.** Investigador somente-leitura reproduz e propõe; eu leio o código
real, escrevo os testes, meço o VERMELHO contra o código do HEAD (trocando só o arquivo de
produção, com backup conferido por hash e restaurado com hash idêntico), corrijo, rodo as famílias
afetadas e mato mutantes. Os limites que o revisor declarou foram mantidos nas mensagens.

| # | Achado | Commit | Vermelho antes | Verificação | Mutação |
|---|---|---|---|---|---|
| 1 | ACL de recurso ignorava lista de ids sob seletor declarado — `{ entidade_ids: [idDeB] }` era autorizado, inclusive com ACL vazia | `1420724f` | 11 casos (13d–13j, 26b–26e) | P05 107/107; `tsc` 0; eslint 0 | 7/7 |
| 2 | Worker descartava frames colados ao `start`: `cancel` sumia, segundo `start` escapava; e o `run_worker` ignorava cancel e erro de protocolo recebidos antes do loop | `2272ac35` | 5 casos do `run_worker` real por asserção; **no spike com o Hermes REAL, o cancel colado terminou num `reply` do provider e o segundo `start` saiu com 0** | pytest 193/193; spike 8/8 em 3 execuções | 12/12 (a 1ª rodada deixou viva a ordem invertida; caso novo a matou) |
| 3 | `request_hash` cobria só o payload: mesma chave e payload noutra conversa, operador, epoch, motivo ou operação voltavam como replay aceito | `5864533f` | 9 casos (pausa 13–17, retomada 25–28) | ver abaixo | 13/14 — ver abaixo |
| 4 | Reentrega concorrente do mesmo comando recebia `epoch_mismatch` (com auditoria falsa na pausa) | `5864533f` | 4 casos de corrida (1–4) | corrida 7/7 em 3 execuções | idem |
| 5 | Recuperação de claim expirado mutava e re-armava o head sob `human`/`pausing` | `c9a03733` | casos 20 e 21; contrato 6→7 | hold 24/24; contrato 13/13 | 5/5, só pela spec de DB real |

**Achados 3 e 4, detalhe.** Identidade do §8.3.2 numa função pura com vetor fixo calculado FORA da
implementação (sha256 em Python sobre JSON canônico escrito à mão). Conflito deixa de devolver o
`command_id` de outra identidade (§8.2.4); replay responde com o `control_id` gravado. Releitura
da chave logo depois do lock do controle, por SELECT simples. Família `hermes-*` de DB real:
**636 passados / 8 pulados** (os 8 do spike). A mutante sobrevivente (H12: a retomada se identificar
como pausa) é **equivalente** — `resume_policy` é sempre não nulo na retomada e nulo na pausa, então
o digest já difere; a remoção do `kind` é morta pela spec unitária. Uma lacuna real apareceu na
primeira rodada (H14: conflito sem auditoria sobrevivia) e foi fechada com asserção nos casos 13 e 25.
Residual caracterizado, não corrigido: C69. Comentário da 141 desatualizado: C68.

**Achado 5, detalhe.** `hermes-*` + `turn-stream*`: **691 passados / 8 pulados**. Família `turn-*`:
184 passados e **3 arquivos em falha que exigem Redis real** (`turn-job-id-real-redis`,
`turn-job-v2-queue-real-redis`, `turn-dlq-replay-operator-real-db`) — **atribuídos por medição**:
com o `turn-repos.ts` do HEAD falham IDÊNTICOS, com `ERR unknown command 'info'` do fakeredis.
Consequência registrada: C70.

**Suíte unitária local em `c9a03733`, medida:** `45 | 10594 | 1321 (11960)` contra
`44 | 10568 | 1296 (11908)`. Pulados **+25** = spike 2 + pausa 6 + retomada 5 + corrida 7 + hold 5.
Passados **+27** esperados (achado 1: 13; identidade: 13; contrato: 1). O resíduo `+1 falha / −1
passado` é UM caso: `audit-exceptions > EXECUÇÃO REAL … o npm responde de verdade`. **Atribuído por
medição, não por nome:** isolado ele falha estável (0,5 s) com o Node 24 global na frente do PATH, e
**passa** (1,1 s) com o `npm` do Node 22 portátil na frente — o arquivo volta às 6 falhas da linha de
base. É o `devEngines` recusando o npm global (§4), efeito da MINHA invocação. Comparação dos
conjuntos por prefixo de 100 caracteres (a rodada foi gravada por `Out-File`, que quebra linha):
nenhum outro teste entrou ou saiu.

**Não feito aqui, e dito:** o pytest do worker não roda no CI (C71a); a janela entre `ready` e a
...
| spike (`tests/hermes-spike`, antes em `tests/reliability`) | 8 | 0 | — | **Desde `2272ac35`: 8/8 em três execuções seguidas**, com os dois casos do achado 2 (cancel e segundo `start` escritos junto com o `start`), vermelhos contra o worker do HEAD. Mudou de lugar em `1aee0da5` (C58) e NÃO roda em job nenhum do CI. `AIAgent` real do SHA pinado (`5d59366`) contra provider **stub** (V-016). **Reexecutado na árvore integrada e depois do C27:** 6/6 nas duas — o registry real aceitou `fixture_echo`. ⚠️ No CI esta spec PULA e reprova a lane `fault injection (#510)` (C58) |
| pytest (`services/hermes_worker`) | 193 | 0 | — | **Desde `2272ac35`: 193/193** (+13 de IPC com frames excedentes, +11 do `run_worker` real antes do loop); fora do CI (C71). V-015 registrou 166; depois foram **169** — a diferença bate com os três casos de custo acrescentados à fixture em `609cc189`, mas isso é INFERÊNCIA (não recoletei a suíte naquele commit). Reexecutado na árvore integrada e depois do C27: 169/169 |
...
- ~~Nenhum CI executado sobre este código~~ — desde 17/09 há CI: PR #766, 12/12 verdes em `1aee0da5`
  (V-050). Os commits dos achados (`1420724f` em diante) ainda não tinham CI quando este registro
  foi escrito. O CI não roda o spike nem o pytest do worker (C58, C71).
