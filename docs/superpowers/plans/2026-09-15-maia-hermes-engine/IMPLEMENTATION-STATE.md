...
| C62 | **SHAs citados nos documentos passaram a apontar para a linha ORIGINAL.** Entradas anteriores citam `7993e563`, `a1c3de3a`, `99845c94`, `989355eb` e outros, que não são ancestrais da linha integrada | `C57-RECONSTRUCAO-E-INTEGRACAO.md` §6 e §8 | **Não reescritos, de propósito:** corrigi-los dentro dos commits reconstruídos mudaria as árvores e destruiria a prova de equivalência. A tabela de mapeamento é a tradução; os originais seguem alcançáveis **só LOCALMENTE**, pelas branches originais e pelos backups — nada disso está no remoto, então para quem revisar no GitHub depois de um push a tabela é a ÚNICA tradução |
| C63 | **O ledger append-only `migrations/RESERVATIONS.md` foi reescrito inteiro por troca de fim de linha, e nenhum gate viu.** O P03.1 (`6517579d`) acrescentou as reservas 139/140 e converteu as 235 linhas existentes de LF para CRLF; o diff contra a `main` mostrava `238/235`, contra a regra do próprio cabeçalho ("Never edit … an existing line") | medido por mim: CR = 0 na `main`, 235 na ponta; `scripts/check-migration-reservations.ts:126` normaliza `\r\n` antes de ler, por isso passava; achado da revisão adversarial, que contou CR por arquivo | **RESOLVIDO** em `a5730a0f`: só fim de linha, diff contra a `main` agora `+3/−0`. Reservas 148/148 e as quatro specs de ledger/numeração 85/85. Varredura dos 95 arquivos alterados pela épica: nenhum com CR na ponta. É a mesma armadilha de CRLF já registrada nesta épica: `schema.ts` foi corrigido em `f0bb928a`, e nos edits do P04 ela foi pega ANTES do commit. Esta escapou até a integração porque o único gate que olha o arquivo normaliza exatamente o que deveria acusar |
| C64 | **Toda verificação local anterior a 17/09 à tarde rodou com dependências que não eram as do lockfile.** A worktree de integração herdou um `node_modules` por junção para o da worktree `github-issue-498-cdbb4e`, que diverge do `package-lock.json` em **63** pacotes de topo | medido por mim com `scratchpad/pr766/lock-vs-instalado.cjs` (versão do lock × `package.json` instalado, só nível de topo): worktree antiga `676 no lock · 90 ausentes · 63 divergentes`; `node_modules` da raiz do repositório `676 · 89 ausentes · 3 divergentes` (`lightningcss` e `lightningcss-win32-x64-msvc` 1.32.0→1.33.0, `picomatch` 4.0.4→4.0.7). Os ausentes são, pela contagem quase igual nos dois, opcionais de outras plataformas — inferência, não conferida pacote a pacote | **Junção trocada para o `node_modules` da raiz** (3 divergências de patch/minor). Não reinstalei nada: o npm do Node portátil (10.9.8) está fora do `devEngines` (§4). As medições anteriores ficam como estão — não há como refazê-las na árvore antiga —, mas nenhuma delas pode ser lida como "com as dependências do lock". A verificação autoritativa é o CI da PR #766, que instala do lockfile |
| C65 | **Correção do C54: a falha `Invalid configuration` sob a config local não vinha da falta de `globalSetup` — vinha de `MAIA_REPO_ROOT`, que eu mesmo definia.** A variável está no namespace da Maia e não existe no contrato de config, e o `loadConfig` recusa chave desconhecida desse namespace | `src/config/validate.ts:232-254` (`MAIA_*`/`FEATURE_*` desconhecida é ERRO em todo profile, regra `contract/unknown`); medido ao reproduzir a fairness da PR #766 (`a11dda7c`): sem `MAIA_REPO_ROOT`, a config local roda `turn-stream-fairness-real-db` e `turn-stream-debounce-real-db`, que antes "morriam em `env.ts:112`"; a config local usa `process.cwd()` quando a variável falta | A regra do C54 (regressão só pela config do projeto; DB real pela local) continua valendo, e a atribuição do V-044 fica como está porque foi feita por eixo pareado. O que muda: o procedimento local **não define** `MAIA_REPO_ROOT` (§9 corrigido). Ainda sem explicação: se a mesma variável também explicava as 408 falhas do P04.6 — não remedi |
| C66 | **Os três `[[allowlists]]` de topo do `.gitleaks.toml` (#326/#328 e #515) são ignorados pelo gitleaks do CI.** O `gitleaks-action@v3` instala a 8.24.3, cujo `ViperConfig` só lê o `[allowlist]` singular no topo; o array de topo só existe a partir da 8.25.0 | lido no fonte da 8.24.3 durante a correção do job de gitleaks da PR #766 (`a0d009a5`), que por isso usa allowlist DE REGRA | **Não corrigido: anterior a esta épica** e fora do escopo. Consequência provável: essas supressões nunca valeram no CI, e o job passava por não haver achado nos caminhos delas — inferência, não conferida contra o histórico de execuções |
| C67 | **A sonda do round-trip do drizzle-kit esconde a causa quando o `generate` sai com 0 sem gerar SQL.** Foi assim que o `TypeError` de serialização de `0n` apareceu no CI da PR #766 como "achei 0 arquivos" | `ff00bb05`: o drizzle-kit 0.31.10 engole a exceção e sai com 0; a sonda descarta a saída de processo que saiu com 0 | **Não corrigido aqui** (script de CI alheio à épica). A melhoria é a sonda falhar fechado mostrando a saída do drizzle-kit. A causa do caso concreto foi corrigida, e `schema-defaults-serializable.spec.ts` impede que ela volte por outro default |
| C68 | **O comentário da migration 141 descreve o `request_hash` como "payload canônico + principal", e nem o código antigo nem o novo fazem isso.** O antigo cobria só o payload (achado 3 da revisão da PR #766); o novo cobre a identidade completa do §8.3.2 | `migrations/141_conversation_control_commands.sql:58` e o `COMMENT` em `:155-156`; `conversationControlCommandDigest` em `conversation-control-repo.ts` | **A 141 não foi editada:** `scripts/migrate.ts` confere checksum, e bancos que já a aplicaram passariam a recusar a migration. O comentário fica desatualizado e a verdade fica no código e aqui. Registrado junto: `ReconcilePauseInput.idempotency_key` é declarado e nunca lido — a reconciliação é idempotente pelo ESTADO, não pela chave |
| C69 | **Residual do achado 4: a mesma chave de idempotência em controles DIFERENTES, ao mesmo tempo, falha com 23505 cru da unique da 141, sem desfecho tipado.** A releitura sob lock não cobre esse caso, porque o lock é de outro controle | caso 7 de `hermes-conversation-control-idempotency-race-real-db.spec.ts`, que CARACTERIZA o comportamento: a segunda transação espera a primeira na unique, falha com `23505` quando ela comita, e o controle dela fica intacto (`bot`/0) | **Falha fechada, aceito como está:** nada é aceito duas vezes e nenhum controle muda. Um desfecho tipado exigiria savepoint ou mapear o 23505 no wrapper `*Tx`. Disparo exige reuso de chave entre conversas diferentes no mesmo instante — bug de cliente, não retry. Hoje as APIs não têm call site de produção |
| C70 | **Consequência do achado 5: um turno `claimed`/`running` de lease vencida, sob hold, agora sobrevive intacto até o resume, e em `bot` é recuperado e executado normalmente.** Antes da correção, o destino dele dependia de ter havido tráfego durante a pausa: com tráfego, virava `retryable` e entrava no descarte do backlog (inclusive um zumbi `running`, o que o §8.2.5 proíbe); sem tráfego, já sobrevivia | `c9a03733`; `heldBacklogForCancellationSql` só seleciona estados reivindicáveis; C55 | **Não resolvido aqui, e não pertence a esta correção:** o fechamento certo é o resume recusar, como pendência não conciliada, enquanto houver turno `claimed`/`running` na stream — unidade dos fences do §8.2.4/P07, junto do C55. A correção removeu o descarte ACIDENTAL; a lacuna que sobra já existia no caminho sem tráfego |
| C71 | **Duas lacunas do worker Python que a correção do achado 2 deixa visíveis.** (a) O pytest do worker (193 casos) não roda em nenhum job do CI; (b) que um `hard_interrupt` feito entre o `ready` e a entrada de `run_conversation` encerre o turno é propriedade do MOTOR, e o spike não isola essa janela | grep de `pytest`/`hermes_worker` em `.github/workflows` (0 ocorrências); `services/hermes_worker/README.md`, seção "Controle que chega antes do loop" | (a) é decisão de CI do dono, como o C58. (b) Lido no Hermes pinado: o interrupt só é limpo ao abortar (`turn_recovery.py:1085-1100`), o que sugere que um interrupt anterior ao loop é visto na primeira checagem — **inferência**, não medida |
...
> **Atualizado em 17/09.** O texto anterior desta seção dizia "Etapa P03, unidade U-P03.2" e estava
> obsoleto desde o P03.3 (apontado pela revisão adversarial da integração). O histórico das unidades
> está na §6.

- **Linha canônica:** `claude/mh-integracao-c57` (worktree `.claude/worktrees/mh-integracao-c57`),
  publicada com autorização do dono como **PR #766** (C57). Merge, release e deploy seguem NÃO
  autorizados.
- **Etapa:** **P04 (controle humano)**, em andamento. P00–P03 concluídos; as fatias de
  contrato/política de P05, P06 e P07 integradas, com vereditos inalterados (PARCIAIS).
- **Unidades fechadas por último:** `U-C57` (integração local), `U-C27` (fixture conforme K-19), a
  correção do ledger (C63), **as quatro correções de CI da PR #766** (V-050) e **os cinco achados da
  revisão da PR #766** (V-051): ACL com lista de ids (`1420724f`), cancel e segundo `start` colados
  ao `start` do worker (`2272ac35`), identidade e corrida da idempotência de pausa/retomada
  (`5864533f`), recuperação de claim expirado sob controle humano (`c9a03733`).
- **Aberto na PR #766:** pergunta ao dono sobre o resto da épica — PRs separadas (recomendado) ou
  continuar nesta — sem resposta ainda. Nada além dos achados e do CI entrou depois da abertura.
- **Próxima unidade desbloqueada:** `U-P04.7a` — fence de controle humano DENTRO da transação do
  commit de outbound (`outbound-outbox-repo.ts`, §8.2.4 linha 2397 e §8.2.3 linha 2373), sob lock
  compartilhado da linha de controle, na ordem de locks controle → turno → outbox. O desfecho do
  turno bloqueado (sem gastar tentativa, sem retry storm) é decisão separada, a registrar antes.
...
- `U-C27` — **concluída e verificada** (`afde96d7` + `95aed007`): nome da tool de fixture conforme
  K-19 sem afrouxar a regra, digest regerado pelo encoder TS, guarda com a função real. ⚠️ A revisão
  adversarial REFUTOU a afirmação de que a guarda cobria "as posições que o protocolo carrega": o
  wire tem quatro chaves de nome de tool e ela lia três (faltava `progress.tool_name`), e tirar a
  leitura das linhas cruas não derrubava caso nenhum. Reforçada em `95aed007` com frames sintéticos
  das quatro chaves e exigência de origem em linha crua; mutação 10/10 (a que antes sobrevivia agora
  morre). Ver V-048 e V-049.
- **Revisão adversarial da integração** (4 frentes independentes, somente-leitura): equivalência
  SUSTENTADA; C27, docs e gates PARCIAIS, sem achado bloqueante. Achados corrigidos: C63 (ledger
  CRLF), guarda do C27, e oito imprecisões de documentação e do rascunho da PR. Ver V-049.
...
# node_modules: junction para o node_modules da RAIZ do repositório (C64: o da worktree
#   github-issue-498-cdbb4e divergia do lockfile em 63 pacotes; o da raiz, em 3).
# PATH: o Node 22 portátil PRIMEIRO. Com o Node 24 global na frente, `npm` é recusado pelo
#   devEngines e `audit-exceptions` ganha uma falha que não é do código (V-051).
...
# Passo 2: família real-db, sem globalSetup. NÃO definir MAIA_REPO_ROOT (C65): a config usa o cwd.
TEST_DB_URL=postgres://maia_test:test1234@127.0.0.1:55432/maia_test_wt_c_users_mendes_documents_10fe483e DATABASE_URL=<o mesmo> REDIS_URL=redis://127.0.0.1:56379 node node_modules/vitest/vitest.mjs run --config <scratchpad>/vitest.integracao-local.config.mts hermes- turn-stream
# pytest do worker (193)
...
# spike (AIAgent real, provider STUB) — mudou de lugar em 1aee0da5 (C58); 8 casos desde 2272ac35.
# O python é o .venv do CHECKOUT PINADO; o hw-venv não tem as dependências do Hermes.
MAIA_HERMES_WORKER_PYTHON=<scratchpad>/hermes/hermes-upstream/.venv/Scripts/python.exe MAIA_HERMES_UPSTREAM=<scratchpad>/hermes/hermes-upstream node node_modules/vitest/vitest.mjs run tests/hermes-spike
