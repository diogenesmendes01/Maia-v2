...
| 2 | `agent.react_loop_tool_reaction` | [`src/runtime/engines/reasoner-stage.ts:408`](../../src/runtime/engines/reasoner-stage.ts) — `sendReaction` | `sendReaction` devolve `void`: um artefato durável nasceria `delivery_unknown` em 100% dos casos e alimentaria a fila **humana** de #633 com ruído | `.catch` que suprime só a reação; a resposta do turno é independente, e uma reação não carrega informação que a resposta já não carregue | `diogenesmendes01` | **carve-out best-effort** — revisão em **2027-03-31** | `sendReaction(` sumir de `reasoner-stage.ts` — por capability nativa (`reaction` virar `PROVIDER_IDEMPOTENCY_NATIVE`) **ou** desfecho terminal sem reconciliação humana |
...
| 2 | `agent.react_loop_tool_reaction` | `reasoner-stage.ts` → `sendReaction(` · some | apagada |
...
**uma só, no callsite** (`sendReaction(` sumir de `reasoner-stage.ts`): os dois
