...
The platform owner's direction: *"nenhuma política periódica nova deve ser ativada sem homologação escrita"*. A sentence in a document does not stop anything, so it is a guard: [`src/ops/privacy/homologation.ts`](../../../src/ops/privacy/homologation.ts) plus [`tests/unit/ops/retention-homologation-guard.spec.ts`](../../../tests/unit/ops/retention-homologation-guard.spec.ts), which runs on every unit pass — and, since PR #737, the same module's evaluator runs in the configuration preflight and in the boot, over the **effective** configuration (see [Reach of the lock](#reach-of-the-lock-declared-state-and-the-effective-configuration-pr-737)).
...
### Reach of the lock: declared state, and the effective configuration (PR #737)

Until PR #737 the lock reached only the state declared in code and the **defaults of the configuration contract**, and said so: an operator exporting `RETENTION_DRY_RUN=false`, or installing a real `RETENTION_POLICY`, activated a policy where no unit test could see it. The platform owner's decision (2026-09-07) was to **reach the effective configuration rather than reduce the promise**: *"use um único avaliador puro no config:preflight e no boot, recebendo a configuração já parseada. Política destrutiva ativa sem homologação deve falhar antes de iniciar workers, sem bypass por MAIA_CONFIG_STRICT_BOOT=false."*

The lock now has three layers, and one evaluator behind the two that reach an environment:

| Layer | Where | What it reaches | Fails how |
|---|---|---|---|
| Declared state | `auditPeriodicPolicies()` + [`retention-homologation-guard.spec.ts`](../../../tests/unit/ops/retention-homologation-guard.spec.ts) | Code and the contract's **defaults** (`ENV_CONTRACT`, read for real) | Red unit test on every CI pass |
| Preflight | `evaluatePeriodicPolicyActivation(values)` in [`src/config/preflight.ts`](../../../src/config/preflight.ts), via `npm run config:preflight` | The **effective environment each Compose container would receive** (`env_file` + `environment:` interpolated), per subset that declares the deciding variables (`runtime`, `backup`, `maintenance`) | Exit 1 **before `docker compose up`**, naming the variable and the policy — never a value |
| Boot | `evaluatePeriodicPolicyActivation(config)` in [`src/index.ts`](../../../src/index.ts), step `config` | The **configuration the process actually received**, already parsed by `src/config/env.ts` | `HOMOLOGATION BOOT REFUSED`, `maia.homologation_boot_refused` in the log, exit 1 — **before Postgres, Redis, the queues and `startWorkers()`**. No worker starts |

**What "active in the effective configuration" means.** The evaluator is pure — no I/O, no `process.env`, no database; it receives the parsed configuration and returns, per policy, `{ active_effective, authorised, violation? }`. Activation follows the variables each policy declares: dry-run ON ⇒ inactive (`RETENTION_DRY_RUN`, `PRIVACY_EXPORT_SWEEP_DRY_RUN`); `retention.class_purge` additionally requires a `RETENTION_POLICY` that parses as approved **and names at least one purgeable class** — the exact condition under which `resolveRetention` starts answering `purgeable: true`. A configuration that does not carry a typed value for a deciding variable is `activation_undeterminable` and fails closed: what is not proven inactive does not pass.

**No bypass.** `MAIA_CONFIG_STRICT_BOOT=false` is the emergency rollback of the configuration **contract** (`loadConfig()` in `src/config/env.ts`) and governs only that. The evaluator does not read the variable, is not handed its value and has no switch of its own; the boot call site does not consult it either. A destructive periodic policy running without written homologation is not a contract inconsistency to be unblocked during an incident — it is the thing the owner's direction forbids. [`docs/runbooks/config-contract.md` §4.1](../../runbooks/config-contract.md) lists it under "what the rollback does NOT turn off"; [`homologation-boot-gate.spec.ts`](../../../tests/unit/runtime/homologation-boot-gate.spec.ts) boots the real `src/index.ts` with the bypass set and a violation present, and proves the refusal and that `startWorkers` was never called.

**What still lies outside all three layers.** A live process whose environment changes after boot without a restart (every deciding variable is `restartRequired: true`, so the next start catches it), and the human review of whether a recorded `WrittenHomologation` is genuine — the lock verifies that one is declared, not that it is true.
...
| | |
|---|---|
| Last updated | 2026-09-07 (PR #737 — by the platform owner's decision of 2026-09-07 the homologation lock **reaches the effective configuration**: one pure evaluator, `evaluatePeriodicPolicyActivation`, runs in `npm run config:preflight` and in the boot of `src/index.ts` before any worker starts, with no bypass via `MAIA_CONFIG_STRICT_BOOT=false`. Earlier, 2026-09-03: this slice was **separated from PR #732 by the platform owner's decision** for its own review: the remaining decisions were **split by owner** (Legal/DPO · Ops · Security · owner still to be assigned); the `privacy.export` seven-day TTL is recorded as **active in production AND still awaiting DPO confirmation**; and *"no new periodic policy is activated without written homologation"* became a guard. The `privacy.tombstone` ratification itself (2026-09-02, reaffirmed 2026-09-03) is delivered in PR #732 and restated here by the decision model. **No period was decided, and nothing new purges anything.** The DPO / Ops / Security acceptances of this slice remain open.) |
