...
- **Nenhum valor de decisão aberta aparece fora de um bloco `DECISÃO ABERTA`, em nenhum dos dois documentos** — travado por [`tests/unit/docs/decisoes-abertas-cobranca.spec.ts`](../../../tests/unit/docs/decisoes-abertas-cobranca.spec.ts), que varre prosa, tabelas e blocos de código, item por item, e reprova também um bloco assinado cujo valor não foi promovido.
