...
| `src/db/repositories/outbound-delivery-repo.ts` | `outboundDeliveryRepo` — o CLAIM, o LEASE e o FENCE da ENTREGA (#632, fatia C da #506). Único lugar que muda o estado de entrega de uma linha do outbox. Toda mutação é UM `UPDATE ... WHERE ... RETURNING` (nunca SELECT-depois-UPDATE: sob READ COMMITTED o perdedor RE-AVALIA o `WHERE` por EvalPlanQual e volta zero linhas) e todo relógio é o do PostgreSQL. O claim NÃO normaliza `sending` de volta para `claimed` (`CASE WHEN status = 'sending' THEN 'sending' ELSE 'claimed' END`) e `markSending` exige `status = 'claimed'`: o sucessor de uma chamada em voo é **estruturalmente incapaz** de reenviar. `recordDeliveryOutcome` PRESERVA a posse quando o desfecho é `accepted_confirmed`, porque `delivered → completed` é fenced pelo mesmo token — soltar ali deixaria a linha eternamente `delivered` sem histórico. `completeDeliveryTx` faz a transição **e** o INSERT em `mensagens` na mesma transação, que é como o histórico é idempotente sem chave de dedupe nova. Fora do barril pelo mesmo ciclo de `auditTx` que exclui `outbound-outbox-repo.ts`. |
| `src/db/repositories/outbound-recovery-repo.ts` | `outboundRecoveryRepo` — as declarações da RECUPERAÇÃO (#633, fatia D da #506). Toda MUTAÇÃO é `UPDATE ... WHERE status = <origem esperada> ... RETURNING` (compare-and-swap): é ISSO, e não um advisory lock, que faz sweepers concorrentes não duplicarem — duas réplicas que decidam o mesmo produzem UM vencedor e um `UPDATE` que volta zero linhas, no lock de row do PostgreSQL. `promoteUnknownToRetryable` é a ÚNICA escrita da fatia que autoriza um efeito externo repetido, e só é chamada quando `reconciliationDisposition` devolve `resend_idempotent` (isto é, quando o provedor honra a chave para aquele `payload_type`). `deadLetterTx` e `rearmManuallyTx` gravam a auditoria por `auditTx` na MESMA transação da transição — uma DLQ sem trilha é um silêncio que ninguém reconstrói. As quatro varreduras são FUNÇÕES exportadas que devolvem o `sql` (`deliverableStatement`, `reconciliationStatement`, `takeoverOnlyStatement`, `scopesWithWorkStatement`), pelo mesmo motivo de `turn-fence-sql.ts`: o teste de EXPLAIN passa a declaração de PRODUÇÃO ao planejador em vez de reescrevê-la. Fora do barril pelo mesmo ciclo de `auditTx` que exclui os dois irmãos. |
