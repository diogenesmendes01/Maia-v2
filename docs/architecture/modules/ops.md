...
| `src/ops/privacy/homologation.ts` | The homologation lock (issue #536): `PERIODIC_POLICIES` with a REQUIRED `ActivationAuthorisation`, the closed `GRANDFATHERED_ACTIVATIONS`, `auditPeriodicPolicies()` over the declared state and the contract defaults, and — since PR #737 — `evaluatePeriodicPolicyActivation(cfg)`, the ONE pure evaluator of the **effective** configuration that both `npm run config:preflight` (`src/config/preflight.ts`) and the boot (`src/index.ts`, before any worker) run. No I/O, no `process.env`, no switch — `MAIA_CONFIG_STRICT_BOOT=false` does not reach it |
...
| Add a data class | `retention/data-classes.ts` + the matrix doc; state its `decision` — either `{ state: 'open', owner, question }` naming the role that owes the answer, or `{ state: 'ratified_by_owner', … }` with the decision and its argument (the union is required, so a class cannot be added without saying which it is; `approval_state` is derived from it). If a subject request should reach it, it must be `scope: 'tenant_agent'` and get an entry in `privacy/execution.ts`'s `PURGE_ORDER` |
...
- `evaluatePeriodicPolicyActivation(cfg)` — the homologation lock over the parsed configuration: per periodic policy, `{ active_effective, authorised, violation? }`. Callers: `runPreflight` (`src/config/preflight.ts`, per contract subset that declares the deciding variables) and the `config` step of `src/index.ts`. `HomologationBootRefusedError` / `formatHomologationBootFailure` are the boot's death message — variable, rule, remediation, never a value
...
- **No periodic policy destroys without written homologation — and the check reaches the configuration the process actually runs with** (issue #536, PR #737). Flipping `RETENTION_DRY_RUN=false`, or installing a `RETENTION_POLICY` that names a purgeable class with dry-run off, is refused by `npm run config:preflight` before `docker compose up` and by the boot before Postgres, Redis, the queues and `startWorkers()`. The refusal names the variable and the policy, never a value. `MAIA_CONFIG_STRICT_BOOT=false` is the contract loader's rollback and does not touch this lock; the only way through is a `WrittenHomologation` recorded in the policy's `authorisation`.
