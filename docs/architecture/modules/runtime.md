...
comprometida no outbox (#506). No caminho feliz, `core.ts` conclui o turno com a
lease viva; se houver falha depois do commit, o hot path preserva
`outbound_pending`, libera a lease e a convergência do `outbound_recovery`
assume depois da expiração. Incluí-lo no índice prenderia a conversa pela
latência do provedor de saída. Consequência honesta: entre `outbound_pending` e
o terminal a stream aceita um novo claim. Isso não é reordenação — QUEM pode ser
reivindicado é o head-of-line (#626), que é outra fatia; esta decide QUANTOS
podem estar ativos.
...
### Outbox de saída (`src/runtime/outbound/`) — issue #506, fatias A–F (#630–#635)
...
| `recovery-contract.ts` | **#633/#635** — contrato PURO da reconciliação, incluindo o vocabulário fechado de resultados e os estados de artefato que permitem concluir o turno                                                     |
...
`contract.ts`, `delivery-contract.ts` e `recovery-contract.ts` são **puros**, na mesma natureza de
`src/runtime/turns/contract.ts`: sem `db`, sem I/O, sem ALS, sem relógio. As
seis fatias da épica estão representadas neste contrato: artefato, commit,
entrega, recovery/DLQ, inventário de egresso e multipart.
...
`src/agent/output-dispatch.ts` — _"log the issue and proceed as if there's no
prior row"_), e enviar e persistir ficavam separados por uma janela de crash. As
...
- **O escopo do turno guarda o handle POR REFERÊNCIA.** `concludeTurn` grava com
  ...
  fechamento síncrono usaria a versão anterior e seria recusado como
  `state_mismatch`. Com recovery ativo e claim válido a convergência posterior
  ainda fecha o turno quando a lease expira, mas com latência e bloqueio FIFO.
...
Cinco operações por tick de `outbound_recovery`
...
4. **convergir o turno** — depois da reconciliação, seleciona
   `outbound_pending` com claim existente e lease expirada; bloqueia primeiro o
   turno e depois todas as partes, exige que todas estejam em `completed |
   failed_terminal | cancelled | dead_letter`; pelo menos uma `completed` é
   necessária para terminalizar, e zero sucesso gera alerta sem inventar
   entrega;
5. **divergência turno↔outbound**, nos dois sentidos, como OBSERVAÇÃO.

A conclusão do último artefato e a conclusão do turno são transações separadas.
Na segunda transação, o CAS `outbound_pending -> completed`, a projeção de
`mensagens.processada_em`, a eleição FIFO do sucessor e a auditoria
`outbound_turn_finalized` são atômicos. O sinal BullMQ acontece somente depois
do commit. Um crash entre as duas transações deixa o turno pendente, mas a perna
por `agent_turns` de `scopesWithWorkStatement` o reencontra no próximo tick
elegível, depois de a lease expirar.

A lease expirada é o selo de quiescência multipart: todo novo commit outbound
exige lease viva, portanto o produtor antigo não pode acrescentar outra parte
depois do vencimento. Claim ou lease ausentes não provam quiescência e falham
fechado.
...
#### Limitações declaradas do recovery

- **Todas as partes finais sem nenhum `completed` não concluem o turno.** O
  sweeper conta esse conjunto numa consulta agregada separada da eleição
  finalizável; assim ele não ocupa o `LIMIT` nem causa starvation. A API
  transacional devolve `no_success` em vez de inventar entrega. O pai continua
  `outbound_pending`, sua idade permanece em
  `maia_outbound_pending_age_seconds`, e o estado atual aparece em
  `maia_outbound_turn_no_success_pending`. Falta uma política própria de
  falha/DLQ para terminalizar esse turno.
- **O outcome deriva dos sucessos comprovados.** Se todo artefato `completed`
  tiver `payload_type = status_fallback`, o turno conclui como
  `fallback_delivered`; qualquer outro sucesso produz `reply_delivered`.
- **Claim ou lease ausente impede a finalização automática.** Isso inclui a
  janela pós-commit no regime `FEATURE_TURN_CLAIM=false`; afrouxar a guarda
  permitiria concluir enquanto um produtor ainda pode acrescentar parte.
- **A consulta de turnos finalizáveis ainda não tem prova real de `EXPLAIN`.**
  Os predicados e o escopo são compilados em teste unitário, mas o plano precisa
  ser medido no PostgreSQL do ambiente de integração antes de escalar volume.
...
3. **Catraca** (#506, auditoria de fechamento) — `assertRatifiedInventory`, em
   ...
   _"inventarie antes de enviar"_; nenhuma dizia _"pare de inventariar"_, e a
...
| `tests/unit/runtime/outbound-recovery-contract.spec.ts`                           | #633/P1: o contrato PURO da recuperação — matriz TOTAL de reenvio, confirmação de risco FAIL-CLOSED, vocabulário fechado de resultados e lista fechada dos estados que permitem concluir um turno (`delivered` fica fora)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tests/unit/db/outbound-turn-finalization-sql.spec.ts`                            | P1 sem Postgres: compila os statements reais e prova escopo `tenant_id + agent_id`, status/fence, sucesso obrigatório, diagnóstico zero-sucesso separado e a perna de descoberta de turnos já convergidos                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `tests/integration/outbound-recovery-reconciliation-real-db.spec.ts`              | #633/P1 com Postgres real, sob tenant PRÓPRIO, entrando por `runOutboundRecoveryForScope` e `rearmOutboundByOperator`: além do ciclo de entrega/DLQ e da janela `delivered -> completed`, cobre finalização do turno, lease viva/nula, multipart, concorrência, outcome de fallback, projeção dos inputs, promoção FIFO pós-commit, auditoria e ausência de starvation por turnos zero-sucesso                                                                                                                                                                                                                                                                                                                                |
