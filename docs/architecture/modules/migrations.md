...
The no-transaction path has the mirror-image guard (issue #733). `splitNoTxStatements` is a parser-free `split(';')` **by design** — the file that needs anything richer belongs in a transaction, where it is sent as one query. That constraint used to live only in a docstring; a `DO $$ … $$;` under the marker was cut inside the body, Postgres received a fragment, and the ledger recorded `42601` as `dirty` with a message that blamed the SQL's syntax. Now `analyzeNoTxSplittability` (`src/migrations/discover.ts`) walks every marked file with the tokeniser's lexical rules — used only to **refuse**, never to split — and `buildMigrationArtifact` reports `no_transaction_unsplittable` for a dollar-quoted body, a string literal containing `;`, or any other shape on which the naive split and the tokeniser disagree on the statement count. `migrate up` checks the artifact **before** constructing anything that could connect (`scripts/migrate.ts`, pre-flight) and the runner re-checks it under the lock, so the file is refused with its cause named and the way out stated (drop the marker). `$$` or `;` that appear only inside `--` comments are not hazards; migrations 096 and 122 rely on that.
...
| A migration that cannot run in a transaction | `-- maia:no-transaction`; keep it to simple `;`-terminated statements. **Enforced** (#733): a dollar-quoted body (`DO $$ … $$`), a string literal containing `;`, or any other shape on which `split(';')` and the lexical tokeniser disagree is refused at discovery as `no_transaction_unsplittable` — drop the marker and let the file run in a transaction instead. See [Fixing `no_transaction_unsplittable`](../../runbooks/migrations.md#fixing-no_transaction_unsplittable). |
