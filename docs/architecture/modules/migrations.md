...
The no-transaction path has the mirror-image guard (issue #733). `splitNoTxStatements` is a parser-free `split(';')` **by design** — the file that needs anything richer belongs in a transaction, where it is sent as one query. That constraint used to live only in a docstring; a `DO $$ … $$;` under the marker was cut inside the body, Postgres received a fragment, and the ledger recorded `42601` as `dirty` with a message that blamed the SQL's syntax. Now `analyzeNoTxSplittability` (`src/migrations/discover.ts`) walks every marked file with the tokeniser's lexical rules plus the backslash escapes of `E'…'` strings (which the tokeniser does not read) — used only to **refuse**, never to split — and `buildMigrationArtifact` reports `no_transaction_unsplittable` for a dollar-quoted body, a `;` inside a string literal, a quoted identifier or a block comment, or a `--` inside any of those (the line stripper runs before the split and would truncate the token). The proof is per token, never a comparison of statement counts: a truncated LAST statement leaves the count unchanged. `migrate up` checks the artifact **before** constructing anything that could connect (`scripts/migrate.ts`, pre-flight) and the runner re-checks it under the lock, so the file is refused with its cause named and the way out stated (drop the marker). `$$` or `;` that appear only inside `--` comments are not hazards; migrations 096 and 122 rely on that.
...
| `tests/unit/migrations/discover.spec.ts` | Ordering (matches the pre-#516 runner), markers, down siblings, the top-level tokeniser, every transaction-envelope verdict, the no-transaction splitter guard (#733: a dollar-quoted body or a `;`-bearing literal under the marker is refused, the same file without the marker is accepted, `$$`/`;` inside `--` comments are not hazards), and the real directory (every `self` file is one complete envelope; every marked file passes the guard; no packaged file changed mode) |
| `tests/unit/scripts/migrate-up-artifact-preflight.spec.ts` | The REAL CLI call site for #733: `scripts/migrate.ts up` reads the artifact first and, on a problem, prints the blockers and exits 1 without calling `runMigrations` or touching the pool; the same file without the marker goes through |
...
| A migration that cannot run in a transaction | `-- maia:no-transaction`; keep it to simple `;`-terminated statements. **Enforced** (#733): a dollar-quoted body (`DO $$ … $$`), a `;` inside a string literal (`E'…'` escapes read), a quoted identifier or a block comment, or a `--` inside any of those is refused at discovery as `no_transaction_unsplittable` — drop the marker and let the file run in a transaction instead. See [Fixing `no_transaction_unsplittable`](../../runbooks/migrations.md#fixing-no_transaction_unsplittable). |
