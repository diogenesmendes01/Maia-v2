...
| `src/onboarding/` | Saga durável de provisionamento (tenant → admin → agente → perfil → packs → papel → canal → pareamento → prontidão → ativação) + readiness canônico por `tenant_id + agent_id` | [onboarding.md](docs/architecture/modules/onboarding.md) |
