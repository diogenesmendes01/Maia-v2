...
export const tracesRouter = router({
  listTraces: protectedProcedure.input(ListTracesSchema).query(async ({ input, ctx }) => {
  ...
  getTrace: protectedProcedure.input(GetTraceSchema).query(async ({ input, ctx }) => {
    const tenantId = resolveTenantId(ctx, input.tenantId);
    ...
    return {
       ...
       */
      attempt_grouping_signed: attempts.length > 0 && attempts.every((a) => a.grouping_signed),
