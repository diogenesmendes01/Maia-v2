...
export const versionsRouter = router({
  listVersions: protectedProcedure.input(ListInputSchema).query(async ({ input, ctx }) => {
        ...
        for (const agentId of agentIds) {
          for (const status of ['proposed', 'active', 'frozen', 'rolled_back'] as const) {
            const rows = await runWithTenantContext(
              ...
              async () => ctx.repos.operationalProfileVersionsRepo.listByStatus(status),
            );
            for (const r of rows) {
              items.push({
                id: r.id,
  ...
  getInUseBy: protectedProcedure.input(GetInUseBySchema).query(async ({ input, ctx }) => {
    const tenantId = resolveTenantId(ctx, input.tenantId);
  ...
  rollback: protectedProcedure.input(RollbackInputSchema).mutation(async ({ input, ctx }) => {
    const tenantId = resolveTenantId(ctx, input.tenantId);
