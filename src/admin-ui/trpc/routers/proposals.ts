...
export const proposalsRouter = router({
  getProposal: protectedProcedure.input(GetInputSchema).query(async ({ input, ctx }) => {
  ...
  approve: protectedProcedure.input(ApproveInputSchema).mutation(async ({ input, ctx }) => {
    const tenantId = resolveTenantId(ctx, input.tenantId);
    ...
    const existingFastCheck = await ctx.repos.proposalApprovalsRepo.listByProposal(
      input.id,
      tenantId,
    );
    ...
    if (
      existingFastCheck.some((a) => a.approver_user_id === ctx.userId && a.decision === 'approved')
    ) {
      throw new TRPCError({
        code: 'CONFLICT',
    ...
    if (dualRequired && allLocks.length === 0) {
      if (
        existingFastCheck.some((a) => a.approver_role === ctx.userRole && a.decision === 'approved')
      ) {
        throw new TRPCError({
          code: 'CONFLICT',
  ...
  reject: protectedProcedure.input(RejectInputSchema).mutation(async ({ input, ctx }) => {
    const tenantId = resolveTenantId(ctx, input.tenantId);
