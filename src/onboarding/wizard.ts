...
export async function startOnboardingRun(input: {
  ...
  const run = await onboardingRunsRepo.create({
    ...
    schema_version: schemaFingerprint(schema.applied_migrations),
    metadata: input.metadata,
  });

  incCounter('maia_onboarding_run_started_total', { kind: input.kind });
...
export async function cancelOnboardingRun(input: {
  ...
  if (outcome.outcome === 'committed') {
    incCounter('maia_onboarding_run_cancelled_total', { reason: input.reason_code.slice(0, 32) });
  }
...
export async function executeOnboardingStep(input: {
  ...
  // Pareamento é o ÚNICO efeito fora da transação da saga. O `command_id` é
  // DERIVADO da (run, passo, chave), então repetir o comando com a mesma chave
  // re-enfileira idempotentemente (contrato de #518: mesma `command_id` ⇒
  // devolve a sessão existente). Um crash entre o enfileiramento e o commit é
  // reparado pelo retry com a MESMA chave.
  if (step === 'start_pairing') {
    const p = payload as { channel_id: string; method: 'qr' | 'code' };
    if (!existing.tenant_id || !existing.agent_id) {
      throw new OnboardingError('scope_mismatch', 'run sem escopo resolvido para pareamento');
    }
    const port = input.deps?.requestPairing ?? defaultPairingPort;
    const res = await port({
      tenant_id: existing.tenant_id,
      agent_id: existing.agent_id,
      channel_id: p.channel_id,
      command_id: deriveCommandId(existing.id, step, idempotency_key_hash),
      method: p.method,
      actor_id: input.actor.actor_id,
      actor_role: input.actor.actor_role,
      correlation_id,
    ...
    if (!res.ok) {
      incCounter('maia_onboarding_step_failed_total', {
        step,
        error_code: res.reason ?? 'pairing_rejected',
      });
      return {
        status: 'denied',
  ...
  const evaluate =
    ...
    ((scope: { tenant_id: string; agent_id: string }) => evaluateAgentReadiness(scope));
  ...
  try {
    outcome = await onboardingRunsRepo.commitStep({
      ...
      apply: async (tx, run): Promise<StepApplication> => {
        switch (step) {
          ...
          case 'start_pairing':
            // O efeito já aconteceu (fora da tx). Aqui só a transição.
            return {
              result: { channel_id: (payload as { channel_id: string }).channel_id },
              summary: { channel_id: (payload as { channel_id: string }).channel_id },
              audit: {
                action: 'onboarding_pairing_started',
                resource_type: 'channel',
                resource_id: (payload as { channel_id: string }).channel_id,
              },
            };
          case 'confirm_channel_ready':
            return applyConfirmChannelReady(tx, run, payload as never);
          case 'evaluate_readiness': {
            readiness = await evaluate(requireRunScope(run));
            return {
          ...
          case 'activate': {
            ...
            readiness = await evaluate(scope);
            if (!readiness.ready) {
              return {
                result: readinessResult(readiness),
                summary: readinessSummary(readiness),
    ...
    incCounter('maia_onboarding_step_failed_total', { step, error_code: code });
    logger.error({ err, step, run_id: input.run_id, correlation_id }, 'onboarding.step_failed');
    throw err;
  }

  observeHistogram('maia_onboarding_step_duration_seconds', (Date.now() - started) / 1000, { step });
  ...
  switch (outcome.outcome) {
    case 'committed':
      incCounter('maia_onboarding_step_completed_total', { step });
      if (step === 'activate') {
        incCounter('maia_onboarding_run_completed_total', { kind: outcome.run.kind });
      }
      return {
        status: 'completed',
    ...
    case 'replayed':
      incCounter('maia_onboarding_idempotency_replay_total', { step });
      return {
        status: 'completed',
        run: toRunView(outcome.run),
        result: outcome.result,
    ...
    case 'denied':
      incCounter('maia_onboarding_step_failed_total', { step, error_code: outcome.code });
      if (readiness) {
        for (const failed of blockingFailures(readiness)) {
          incCounter('maia_agent_readiness_failed_total', { check_code: failed.code });
        }
      }
      return {
        status: 'denied',
