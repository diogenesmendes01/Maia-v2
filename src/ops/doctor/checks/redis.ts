...
export const redisConnectivityCheck: DoctorCheck = {
  ...
  async run(ctx: DoctorContext): Promise<DoctorResult> {
    if (!ctx.redis) return skip('nenhum handle de Redis foi aberto para esta execução');
...
export const persistenceCheck: DoctorCheck = {
  ...
  async run(ctx: DoctorContext): Promise<DoctorResult> {
    ...
    if (!aof && lastBgsave === 'unknown') {
      return {
        status: 'warn',
        summary: 'sem AOF e sem evidência de snapshot RDB — o keyspace não sobrevive a um restart',
        evidence,
        remediation: [
    ...
    return pass(aof ? 'AOF habilitado' : `RDB (último bgsave: ${lastBgsave})`, evidence);
  },
};
