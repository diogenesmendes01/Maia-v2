...
export function createReplayToolIO(
  ...
  for (const c of snapshot.recorded_calls) porChave.set(`${c.name}\u0000${c.args_digest}`, c);
  const usadas = new Set<string>();

  return {
    invokeTool: async (call: EngineToolCallV1): Promise<EngineToolReplyV1> => {
      const chave = `${call.name}\u0000${digestArgs(call.args)}`;
      const gravada = porChave.get(chave);
      if (gravada === undefined) {
        naoGravadas.push({ kind: 'tool_not_recorded', call_id: call.call_id, tool: call.name });
    ...
    naoUsadas: () => [...porChave.entries()].filter(([k]) => !usadas.has(k)).map(([, v]) => v),
  };
}
...
export async function runShadowEvaluation(input: {
  ...
  if (start.kind === 'accepted') {
    ...
    if (obs.kind === 'terminal') shadowStop = obs.proposal.stop;
  }
  ...
  if (shadowStop !== null) {
    if (shadowStop.kind !== snapshot.production_stop.kind) {
      ...
      divergences.push({ kind: 'reply_text', note: 'texto diferente com o mesmo desfecho' });
    }
  }
  ...
  const report: ShadowReportV1 = {
    ...
    shadow_stop_kind: shadowStop?.kind ?? 'not_produced',
    divergences,
