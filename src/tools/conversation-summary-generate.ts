 ...
 *   - side_effect: 'none' — reads recent messages (or the caller-provided
 *     `history`) and returns text; it does NOT persist and does NOT close the
 *     conversation (that is the worker's job).
...
export const conversationSummaryGenerateTool: Tool<typeof inputSchema, typeof outputSchema> = {
  ...
  handler: async (args, ctx) => {
    ...
    //   decisions                   → main_actions (what was done/committed)
