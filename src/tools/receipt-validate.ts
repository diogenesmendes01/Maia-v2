...
export const receiptValidateTool: Tool<typeof inputSchema, typeof outputSchema> = {
  ...
  handler: async (args, ctx) => {
    ...
    if (!parsed.beneficiario_nome && !parsed.beneficiario_chave_pix && !parsed.beneficiario_documento)
      warnings.push('missing_recipient_identification');

    const authenticity = classifyAuthenticity(confidence, warnings.length);

    return {
      valid: amount !== undefined && Boolean(date) && authenticity !== 'suspicious',
