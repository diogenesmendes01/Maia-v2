...
d('#510 FI-08/FI-09 — varredura concorrente e job retido', () => {
  beforeAll(async () => {
    ...
    fila = new Queue(FILA, {
      connection: new IORedis(env.estado.redisUrl, { maxRetriesPerRequest: null }),
    });
  }, 300_000);
  ...
  // FI-08
  // ─────────────────────────────────────────────────────────────────────────
  it('FI-08 — dois sweepers sobre o MESMO lote: uma decisão por linha, um job por linha', async () => {
    ...
    for (const viva of [vivaA, vivaB]) {
      ...
      expect(l.status, 'uma linha abaixo do teto foi morta pela varredura').not.toBe('dead_letter');
  ...
  // FI-09
  // ─────────────────────────────────────────────────────────────────────────
  it('FI-09 — job determinístico RETIDO em failed: a varredura rearma; o job VIVO é preservado', async () => {
    ...
    await fila.add(
      'deliver',
      { version: 1, outbound_id: bloqueada.outbound_id },
      {
        jobId: idBloqueada,
      },
    );
    ...
    await fila.add(
      'deliver',
      { version: 1, outbound_id: comJobVivo.outbound_id },
      {
        jobId: idVivo,
      },
    );
