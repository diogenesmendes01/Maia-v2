...
test.describe('Drift & Incidents — Tela 4', () => {
  test('loads drift page with all 3 incident cards', async ({ page }) => {
    ...
    await expect(page.locator('text=PEP blocks')).toBeVisible();
    await expect(page.locator('text=Budget alerts')).toBeVisible();
    await expect(page.locator('text=Regression alerts')).toBeVisible();
  });
});
