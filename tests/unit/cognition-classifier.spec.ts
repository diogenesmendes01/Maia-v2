...
vi.mock('@/lib/claude.js', () => ({
  callLLM: vi.fn(async ({ messages }) => {
    ...
    if (user.includes('FATO_X'))
      return { content: JSON.stringify({ type: 'fato', content: 'X', scope: 'agent' }) };
    ...
    if (user.includes('REGRA_X'))
      return {
        content: JSON.stringify({
          ...
          acao: 'Y',
          tipo: 'classificacao',
          confianca_sugerida_llm: 0.7,
        }),
      };
    if (user.includes('LACUNA_X'))
      return {
        content: JSON.stringify({
          type: 'lacuna',
    ...
    if (user.includes('DESCARTE_X'))
      return { content: JSON.stringify({ type: 'descarte', reason: 'irrelevant' }) };
