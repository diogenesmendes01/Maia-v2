 ...
 *
 * This unit test compiles the repo's ACTUAL WHERE clause with the Pg dialect and
 * asserts the tenant/agent predicate + bound params are present — so the
 * invariant is proven on CI lanes WITHOUT a database. Mirrors the established
 * `*-repo-scope` DB-free pattern.
 */
