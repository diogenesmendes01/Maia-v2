...
import { nationalMovingHolidays, NATIONAL_FIXED } from '../../src/lib/national-holidays.js';

describe('national holidays', () => {
  it('moving holidays 2026', () => {
    const list = nationalMovingHolidays(2026);
    const map = Object.fromEntries(list.map((h) => [h.name, h.date.toISOString().slice(0, 10)]));
