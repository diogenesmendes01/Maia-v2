...
const { enqueueAgentMock, listPairsMock, listUnprocessedMock } = vi.hoisted(() => ({
...
beforeEach(() => {
  ...
  listUnprocessedMock.mockImplementation(async () => {
...
describe('Issue #345 — runMessageRecovery is per-tenant scoped (no default/default leak)', () => {
  it('MULTI-TENANT — inner runs once per enumerated tuple under its own context', async () => {
    ...
    expect(listPairsMock).toHaveBeenCalledTimes(1);
  ...
  it('EMPTY enumeration → no-op (inner read never fires)', async () => {
    ...
    expect(listUnprocessedMock).not.toHaveBeenCalled();
  ...
  it('the dispatcher passes the SAME staleness threshold (STUCK_AFTER_MS) to enumeration', async () => {
    ...
    expect(listPairsMock).toHaveBeenCalledWith(2 * 60 * 1000);
  });
});
 ...
 */
describe('Issue #345 — message-recovery reads ONLY the current tenant/agent', () => {
  beforeEach(() => {
    vi.resetModules();
    storeRows = [
