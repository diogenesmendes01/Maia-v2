 ...
 *
 * The registry is imported STATICALLY here (importing it costs seconds under
 * load), and only the rollback-flag block pays for `vi.resetModules()` plus a
 * re-import.
 */
...
import {
  ...
  describeExposedSchemas,
  REGISTRY,
} from '../../../src/tools/_registry.js';
import { _resetSchemaCacheForTests } from '../../../src/tools/schema-json.js';
...
function byEntityWith(acoes: string[]): Map<string, ResolvedPermission> {
  return new Map([['e1', { profile: { acoes } } as unknown as ResolvedPermission]]);
}

const OWNER = () => byEntityWith(['*']);

describe('#509 exposure — strict schemas reach every model-facing surface', () => {
  it('getToolSchemas returns strict, closed schemas (never the generic stub)', () => {
  ...
  it('getAgentToolSchemas returns strict schemas for the granted subset', () => {
  ...
  it('getToolSchemasByName (tool_mediated) returns strict schemas', () => {
  ...
  it('describeExposedSchemas reports a deterministic set hash + byte budget', () => {
  ...
  it('unknown tool names are skipped by describeExposedSchemas (fail-closed, no throw)', () => {
...
describe('#509 exposure — fail-closed on a broken contract', () => {
  it('a tool whose schema cannot be converted is DROPPED, never made permissive', async () => {
    ...
    const victim = REGISTRY.query_balance!;
    ...
    try {
      const names = getToolSchemas(OWNER()).map((s) => s.name);
