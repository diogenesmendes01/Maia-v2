 ...
 *   - REGISTRY contains all 8 adapters.
 *   - receipt_validate delegates through the parse_receipt / parseImage path
 *     (vision cache keyed by `parse_receipt`; no second vision call site).
 ...
 *   - bank_account_validate returns missing fields + warnings for incomplete data.
...
// ===========================================================================
describe('receipt_validate — delegates through parse_receipt/parseImage path', () => {
  ...
  it('serves a parse_receipt CACHE HIT without any vision call', async () => {
  ...
  it('resolves the attachment from the conversation metadata when no path is given', async () => {
    ...
    getCachedVisionMock.mockResolvedValueOnce({ tipo: 'pix', valor: 10, data: '2026-01-01', confianca: 0.6 });
    const { receiptValidateTool } = await import('@/tools/receipt-validate.js');
...
describe('case_risk_classify — composes classifyTurnRisk', () => {
  it('declares a classification-only contract', async () => {
    ...
    expect(caseRiskClassifyTool.operation_type).toBe('parse_only');
...
// ===========================================================================
describe('bank_account_validate — local structural validation', () => {
...
describe('conversation_summary_generate — wraps the shared summarizer', () => {
  it('declares a read-only contract and maps to the boleto shape', async () => {
    ...
    expect(conversationSummaryGenerateTool.operation_type).toBe('parse_only');
