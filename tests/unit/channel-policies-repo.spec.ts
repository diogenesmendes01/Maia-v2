...
import type { SwitchBehavior, AnnounceMode } from '@/types/enums.js';
      ...
      getByChannelId: vi.fn(async (channel_id: string) => {
        ...
        return (
          Object.values(policiesState).find(
            (r) =>
              r.tenant_id === tenant_id && r.agent_id === agent_id && r.channel_id === channel_id,
          ) ?? null
        );
      }),
      update: vi.fn(async (id: string, patch: Partial<PolicyRow>) => {
        const tenant_id = getCurrentTenant();
        const agent_id = getCurrentAgent();
