...
describe('Requirement 3 — month_end_policy', () => {
  it('skip_invalid_month: skips Feb / Apr / Jun / Sep / Nov', () => {
    // Feb 2026
    expect(
      computeNext(RULE, new Date('2026-02-01T03:00:00Z'), 'skip_invalid_month').toISOString(),
    ).toBe('2026-03-31T12:00:00.000Z');
    // Apr 2026 (30 days)
    expect(
      computeNext(RULE, new Date('2026-04-01T03:00:00Z'), 'skip_invalid_month').toISOString(),
    ).toBe('2026-05-31T12:00:00.000Z');
    // Jun 2026 (30 days)
    expect(
      computeNext(RULE, new Date('2026-06-01T03:00:00Z'), 'skip_invalid_month').toISOString(),
    ).toBe('2026-07-31T12:00:00.000Z');
    // Sep 2026 (30 days)
    expect(
      computeNext(RULE, new Date('2026-09-01T03:00:00Z'), 'skip_invalid_month').toISOString(),
    ).toBe('2026-10-31T12:00:00.000Z');
    // Nov 2026 (30 days)
    expect(
      computeNext(RULE, new Date('2026-11-01T03:00:00Z'), 'skip_invalid_month').toISOString(),
    ).toBe('2026-12-31T12:00:00.000Z');
  });

  it('last_day_of_month: Feb 2026 → Feb 28; Feb 2028 leap → Feb 29; Apr → Apr 30', () => {
    expect(
      computeNext(RULE, new Date('2026-02-01T03:00:00Z'), 'last_day_of_month').toISOString(),
    ).toBe('2026-02-28T12:00:00.000Z');
    expect(
      computeNext(RULE, new Date('2028-02-01T03:00:00Z'), 'last_day_of_month').toISOString(),
    ).toBe('2028-02-29T12:00:00.000Z');
    expect(
      computeNext(RULE, new Date('2026-04-01T03:00:00Z'), 'last_day_of_month').toISOString(),
    ).toBe('2026-04-30T12:00:00.000Z');
  });

  it('nearest_next: Feb → Mar 1; Apr → May 1', () => {
    expect(computeNext(RULE, new Date('2026-02-01T03:00:00Z'), 'nearest_next').toISOString()).toBe(
      '2026-03-01T12:00:00.000Z',
    );
    expect(computeNext(RULE, new Date('2026-04-01T03:00:00Z'), 'nearest_next').toISOString()).toBe(
  ...
  it('nearest_previous: Feb 2026 → Feb 28; Feb 2028 leap → Feb 29; Apr → Apr 30', () => {
    expect(
      computeNext(RULE, new Date('2026-02-01T03:00:00Z'), 'nearest_previous').toISOString(),
    ).toBe('2026-02-28T12:00:00.000Z');
    expect(
      computeNext(RULE, new Date('2028-02-01T03:00:00Z'), 'nearest_previous').toISOString(),
    ).toBe('2028-02-29T12:00:00.000Z');
    expect(
      computeNext(RULE, new Date('2026-04-01T03:00:00Z'), 'nearest_previous').toISOString(),
    ).toBe('2026-04-30T12:00:00.000Z');
  });
