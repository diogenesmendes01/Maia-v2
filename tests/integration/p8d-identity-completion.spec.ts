...
vi.mock('@/db/repositories.js', async () => {
  ...
  return {
    ...
    operationalProfileVersionsRepo: {
      create: vi.fn(
        async (input: {
          ...
          const { LearnedVoiceModifierSchema } =
            await import('@/identity/learned-voice-modifier.js');
      ...
      getActive: vi.fn(
        async () =>
          findActive('default', 'default') as unknown as AgentOperationalProfileVersion | null,
      ),
      getById: vi.fn(
        async (id: string) =>
          (profilesState[id] ?? null) as unknown as AgentOperationalProfileVersion | null,
      ),
      listByStatus: vi.fn(
        async (status: string) =>
          Object.values(profilesState).filter(
            (r) => r.status === status,
      ...
      transition: vi.fn(
        async (args: {
          ...
          return {
            ok: true as const,
            updated: updated as unknown as AgentOperationalProfileVersion,
          };
        },
      ),
    ...
    tenantsRepo: {
      list: vi.fn(async () => [
        { id: 'default', nome: 'default', status: 'active', created_at: new Date() },
      ]),
    },
  };
});
...
describe('P8d Identity Completion (6 cenários integration)', () => {
  ...
  it('cenário 4: profile frozen → identity-slice-builder retorna null (defesa runtime)', async () => {
    await runWithTenantContext({ tenant_id: 'default', agent_id: 'default' }, async () => {
      ...
      const { buildIdentitySlice } =
        await import('@/runtime/context-assembly/slice-builders/identity-slice-builder.js');
