...
const { estado, canal, tts } = vi.hoisted(() => {
  ...
  process.env.RUNTIME_TRACE_HMAC_MASTER_SECRET ??= 'fixture-sonda-634-sem-valor-real';
  return {
...
d('#634 — mídia de saída durável (Postgres + filesystem reais)', () => {
  ...
  it('o ramo de VOZ commita um artefato durável com storage_object', async () => {
    ...
    await comoOWorkerDono(() => dispatchOutput(ctxDeVoz('trezentos reais este mês')));
  ...
  it('o objeto referenciado contém exatamente o áudio que foi sintetizado', async () => {
    ...
    await expect(
      comoOWorkerDono(() => resolveOutboundMediaPath(linha!.payload_json.media as MediaRef)),
  ...
  it('depois de um envio INCERTO o PDF temporário sumiu e a referência commitada ainda resolve', async () => {
    ...
    await expect(comoOWorkerDono(() => dispatchOutput(ctx))).rejects.toBeInstanceOf(
  ...
  it('a MESMA chave de objeto não resolve sob outro tenant, e resolve sob o dono', async () => {
    const ref = await runWithTenantContext({ tenant_id: TENANT, agent_id: AGENT }, () =>
      putOutboundMedia({
    ...
    const doDono = await runWithTenantContext({ tenant_id: TENANT, agent_id: AGENT }, () =>
    ...
    await expect(
      runWithTenantContext({ tenant_id: TENANT, agent_id: AGENT }, () =>
        resolveOutboundMediaPath({
          kind: 'storage_object',
          bucket: 'outro-store',
          object_key: ref.ref.kind === 'storage_object' ? ref.ref.object_key : '',
        }),
      ),
  ...
  it('o apagamento por titular remove os objetos DELE e preserva os de outro titular', async () => {
    ...
    try {
      const doTitular = await runWithTenantContext({ tenant_id: TENANT, agent_id: AGENT }, () =>
        putOutboundMedia({
      ...
      const doOutro = await runWithTenantContext({ tenant_id: TENANT, agent_id: AGENT }, () =>
        putOutboundMedia({
