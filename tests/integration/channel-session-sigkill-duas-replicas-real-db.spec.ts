...
} from '../reliability/harness/process-supervisor.js';
...
d('#513 (fatia D) — duas réplicas de PROCESSO e SIGKILL (Postgres real)', () => {
  ...
  it('réplica B (outro processo, outro pid) é RECUSADA enquanto a lease de A está viva', async () => {
  ...
  it('SIGKILL em A: nenhum release rodou, a posse continua registrada nele — e só depois do prazo B assume, com fence ESTRITAMENTE maior', async () => {
    const a = subirReplica('replica-a');
    const pa = await prontidaoDe(a);
  ...
  it('o MESMO binário, encerrado com SIGTERM, DEVOLVE a linha — é isto que torna o caso do SIGKILL não-vazio', async () => {
    const a = subirReplica('replica-a');
    const pa = await prontidaoDe(a);
    expect(pa.held).toBe(true);
  ...
  it('SIGKILL entre uma batida e a seguinte: quem manda no failover é o prazo que o ÚLTIMO heartbeat gravou', async () => {
  ...
  it('o dono ANTIGO que volta com o token velho na mão é recusado nas QUATRO operações fenced', async () => {
    const a = subirReplica('replica-a');
    const pa = await prontidaoDe(a);
    expect(pa.held).toBe(true);
    expect(pa.fencing_token).toBe(1);
