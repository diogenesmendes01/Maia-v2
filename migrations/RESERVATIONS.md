...
137 | 137_channel_session_leases.sql | issue 513 (fatia A da topologia): a POSSE de uma sessao de canal como dado do banco — channel_session_leases com channel_id como PK (o "no maximo um dono por linha" vira constraint, nao convencao), fencing_token bigint MONOTONICO por canal (a linha nunca e apagada; release marca status e PRESERVA o token, entao um dono antigo que volta nunca reapresenta um token que ainda valha), owner_instance_id, acquired_at/heartbeat_at/lease_expires_at e status. O claim e um unico statement atomico (INSERT ... ON CONFLICT DO UPDATE ... WHERE lease vencida OU liberada OU mesmo dono, RETURNING): duas replicas produzem UM vencedor e zero. O relogio e o DO BANCO (now()), nunca o da replica — clock skew entre containers e risco declarado da issue. CHECK fail-closed contra o literal default e CHECK lease_expires_at > acquired_at. Indice parcial CROSS-TENANT da varredura de lease vencida, como o da 114 e o da 131. Tabela NOVA e VAZIA: sem CONCURRENTLY, com envelope BEGIN/COMMIT (fora da armadilha da 658). Difere de agent_turns.claim_token (uuid) de proposito: a issue exige que o token AUMENTE a cada takeover, e uuid nao ordena
