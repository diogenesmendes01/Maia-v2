...
124 | 124_agent_turns_stream_exclusion.sql | issue 625 (fatia B da 505): exclusao de no maximo UM turno ativo por stream garantida no banco — unique parcial CONCURRENTLY (tenant_id, agent_id, stream_key) WHERE stream_key IS NOT NULL AND status IN ('claimed','running'); o escopo por tenant/agent e parte da constraint (sem ele duas tenants com a mesma stream_key competiriam), e o rollback e derrubar o indice, nao reverter o escalonador (123 esta reservada por outra branch em voo nesta mesma leva)
125 | 125_tool_request_proposals.sql | issue 636 (fatia A da 471): pedido de ferramenta — ledger agent_capability_gap_observations (situacoes com link de trace e janela de frequencia, que o contador frequency_score nao responde) + capability_type 'tool_request' em capability_proposals com CHECK que EXIGE a marcacao de rascunho (spec_kind + contract_status='draft_proposal_not_in_force'), para que o contrato Zod proposto nao possa ser confundido com contrato vigente (123 e 124 estao reservadas por outras branches em voo nesta mesma leva)
126 | 126_agent_turns_stream_head_live.sql | issue 626 (fatia C da 505): indice PARCIAL de suporte ao head-of-line — (tenant_id, agent_id, stream_key, first_ingress_seq) WHERE stream_key IS NOT NULL AND status NOT IN (terminais), CONCURRENTLY. O indice largo da 122 poe `status` DEPOIS de `first_ingress_seq`, entao a desigualdade vira filtro e o `NOT EXISTS` percorre o historico inteiro da conversa a cada claim; aqui os terminais saem do indice e a sonda passa a custar o BACKLOG, nao o trafego. Nao substitui a 122 (o rollback precisa ser "derrube um indice", nunca "reconstrua um"). 123 (issue 631), 125 (issue 636) estao reservadas por outras branches em voo nesta mesma leva
127 | 127_agent_turns_stream_promotion.sql | issue 627 (fatia D da 505): a DECISAO de promocao do sucessor, persistida — agent_turns.promoted_at (turno eleito para avancar; e o dado que permite ao recovery reconciliar "commit feito, enqueue nao feito", porque a BullMQ e wake-up e nao fonte de verdade) e promoted_by_turn_id (o predecessor terminal que promoveu; NULL = re-arme vindo da recuperacao de claim expirado da propria stream). Sem indice novo e sem CONCURRENTLY: ADD COLUMN nullable e metadata-only, e a coluna e lida na linha ja encontrada pelo varredor, nunca como predicado de busca. 123 (issue 631), 125 (issue 636) estao reservadas por outras branches em voo nesta mesma leva
129 | 129_tool_request_aggregation.sql | issue 637 (fatia B da 471): agregacao de pedidos de ferramenta por similaridade — tool_request_aggregates (UM pedido com contador, escopado por tenant+agent, com metrica/limiar/assinatura_version gravados na linha) + tool_request_aggregate_members (ledger append-only com original_spec inteiro e detached_at em vez de DELETE, para que a fusao nunca apague a evidencia dos pedidos originais); sem coluna vector e sem indice ivfflat/hnsw, com o motivo em src/cognition/tool-request/similarity.ts (123-128 estao reservadas por outras branches em voo nesta mesma leva)
130 | 130_agent_turns_debounce_window.sql | issue 628 (fatia E da 505): a JANELA DE DEBOUNCE como dado do PostgreSQL — agent_turns.debounce_window_opened_at (ancora do teto max_hold, sobrevive ao reinicio do processo que abriu a janela), debounce_deadline_at (o relogio PERSISTENTE: a autorizacao de fechar e `<= now()` avaliada no banco, nunca um Date.now() de replica), debounce_closed_at (o fechamento UNICO por compare-and-swap: o UPDATE exige IS NULL, entao duas replicas produzem uma linha e zero) e debounce_batch_size (evidencia forense da composicao + maia_stream_debounce_batch_size). Mais o parcial CROSS-TENANT agent_turns_debounce_due_idx (debounce_deadline_at) WHERE deadline IS NOT NULL AND closed_at IS NULL, CONCURRENTLY — a pergunta do varredor ("quais streams tem janela vencida?") e feita FORA de contexto de tenant, como a de lease vencida da 114. Arquivo NAO atomico de proposito (CONCURRENTLY exige no-transaction); os dois statements sao idempotentes e o estado intermediario (colunas sem indice) e correto, so lento. 123 (issue 631), 125 (issue 636), 128, 129 (issue 637) estao reservadas por outras branches em voo nesta mesma leva
...
132 | 132_tool_request_triage.sql | issue 638 (fatia C da 471, fecha a epica): triagem no console — tool_request_issues (a RESERVA que torna o aceite idempotente antes de a chamada externa sair: UNIQUE por agregado + idempotency_key deterministica que viaja no corpo da issue como marcador, e o corpo gravado no aceite para que o relayer nao possa reescrever o que o dono aprovou), resolved_at/resolved_reason/resolved_tool_name em agent_capability_gaps (o gap fecha por fato verificavel — tool no registro E concedida ao tenant+agent —, nunca por caixa marcada no console) e tool_request_notifications (o aviso ao agente como linha auditavel, UNIQUE por gap); nenhuma coluna de credencial, porque o token do GitHub so existe no subset do servico runtime (130 e 131 estao reservadas por outras branches em voo nesta mesma leva)
133 | 133_agent_stream_blocks.sql | issue 629 (fatia F da 505, fecha a epica): a STREAM BLOQUEADA por poison message como DADO — agent_stream_blocks (uma linha ATIVA por (tenant, agent, stream_key), com reason/category/blocked_by_turn_id/error_code e o ciclo de desbloqueio auditado em unblocked_at/by/reason). E a segunda saida da politica de poison/DLQ que a issue-mae exige escolher CONSCIENTEMENTE: dead_letter que LIBERA o sucessor (disponibilidade as custas da semantica) ou stream BLOQUEADA para intervencao (semantica as custas da conversa). Ate aqui so a primeira existia, e por OMISSAO — dead_letter e terminal, sai do predicado de head-of-line, e o sucessor virava reivindicavel sem ninguem ter decidido. Unique parcial agent_stream_blocks_active_uq WHERE unblocked_at IS NULL = idempotencia do bloqueio no BANCO (duas conclusoes simultaneas da mesma stream produzem UMA linha); indice de historico por (tenant, agent, stream_key, blocked_at DESC). Tabela NOVA e VAZIA, entao SEM CONCURRENTLY e com envelope BEGIN/COMMIT — nao esta exposta a armadilha da issue 658. CHECK fail-closed contra o literal 'default' (uma stream bloqueada sob default seria bloqueio global disfarcado) e CHECK que recusa desbloqueio sem autor e justificativa. 128 foi reservada e NAO usada; 123 (issue 631), 125 (issue 636), 129 (issue 637), 131 (issue 633), 132 (issue 638) estao reservadas por outras branches em voo nesta mesma leva
...
136 | 136_bootstrap_credentials.sql | issue 519 (bootstrap global): a credencial de primeiro bootstrap como DADO do banco, nao arquivo — bootstrap_credentials guarda SO o hash (sha256 do segredo de 128 bits), com expires_at, failed_attempts/locked_until (rate limit e lockout) e consumed_at. A invalidacao e um compare-and-swap (UPDATE ... WHERE consumed_at IS NULL), entao duas tentativas simultaneas produzem UM consumo e zero. O unique PARCIAL bootstrap_credentials_unconsumed_uq WHERE consumed_at IS NULL garante no maximo UMA credencial viva por vez, e o marcador bootstrap_completions torna o bloqueio pos-bootstrap um FATO do banco, nao uma checagem de aplicacao que a proxima replica poderia perder. Tabelas NOVAS e VAZIAS: sem CONCURRENTLY, com envelope BEGIN/COMMIT. Substitui o INSERT manual em app_users com tenant_id='default' que docs/admin-ui-deploy.md documentava, e que violava o invariante de nenhum fallback `default` em path dinamico
137 | 137_channel_session_fencing.sql | issue 513 (fatia A da topologia): o FENCING TOKEN da posse de sessao de canal — channel_line_state.session_fencing_token (bigint, monotonico por canal). NAO e tabela nova de proposito: a 103 ja guarda a posse em session_owner_instance + session_owner_lease_expires_at, com escritor vivo (channel-pairing-worker) e consumidor vivo (o enderecamento de disable/repair a replica que segura o socket); uma segunda tabela daria DOIS donos declarados do mesmo fato, e eles divergiriam, porque renewSessionLeases e last-writer-wins por desenho declarado ("Nao usa CAS por dono") — a replica B se carimbaria como dona enquanto A segurasse a lease, e o comando voltaria a ser consumido pela replica ERRADA (o P1 fechado pela review da PR 528). O que faltava nao era tabela, era o fence. bigint e nao uuid porque a issue exige que o token AUMENTE a cada takeover e uuid nao ordena. O token sobrevive ao release (a posse zera o dono, nunca o contador), e as rows de channel_line_state nunca sao apagadas, entao o contador e monotonico por canal para sempre. Backfill explicito das rows que ja tem dono (nasceriam com 0 e violariam a CHECK) antes do CHECK de coerencia dono=>prazo+fence, no espirito do CHECK de claim/lease da 121. Indice parcial CROSS-TENANT da varredura de posse orfa, como o da 114 e o da 131. ADD COLUMN com DEFAULT constante e metadata-only desde o PG 11; o indice sai SEM CONCURRENTLY porque a tabela tem UMA row por linha de WhatsApp, entao a varredura sob lock e instantanea e CONCURRENTLY so traria a armadilha de indice invalido da 658
138 | 138_objective_tasks_lease_fencing.sql | issue 469 (fatia A do work loop): lease, fencing e reaper em objective_tasks — claim_token/claimed_by/claimed_at/lease_expires_at/claim_attempts. O claim de 088 marcava status='running' e NADA MAIS: um SIGKILL entre o claim e transitionTask deixava a tarefa presa em running para sempre, sem dono e sem prazo, e o comentario do runTick em src/workers/index.ts ja afirmava (falsamente) que objective_execute era single-flight por lease de DB. As colunas sao aditivas e nullable (metadata-only), o indice de varredura de lease vencida e PARCIAL e CROSS-TENANT (status='running' AND lease_expires_at IS NOT NULL), no mesmo padrao do indice de lease vencida da 114 e da 131, porque a pergunta do reaper ('quais tarefas perderam o dono?') e feita FORA de contexto de tenant. claim_attempts existe para que o reaper nao reanime para sempre uma poison task que derruba o processo: acima do teto ela vai para failed, nao para pending. Tabela recem-criada e vazia em producao (o grupo console nasce desligado), entao sem CONCURRENTLY e com envelope BEGIN/COMMIT
139 | 139_approval_requests_scope_uq.sql | escopo composto em approval_requests para a FK do journal de execucao (spec maia-hermes 5.6.2)
140 | 140_engine_run_journal.sql | journal duravel de execucao do engine + controle de conversa (spec maia-hermes 5.6.2 e 8.2.1)
141 | 141_conversation_control_commands.sql | idempotencia de comando de controle humano da conversa (spec maia-hermes 8.2.3 passo 2 e 8.2.4): a chave que torna pause/resume repetivel sem incrementar epoch duas vezes, com request_hash para que a MESMA chave com payload divergente seja conflito e nao ultima-escrita-vence; expected_epoch/result_epoch para o CAS, principal atual e desfecho tipado. Serve tambem de outbox de intencao de cancelamento/reconciliacao. Tabela NOVA e VAZIA: sem CONCURRENTLY, envelope BEGIN/COMMIT. Sem FK para app_users porque conversation_controls (140) tambem nao tem
144 | 144_hermes_inference_ledger.sql | ledger do gateway de inferencia Hermes (spec maia-hermes 9.1 e 9.2): engine_inference_grants (credencial curta do run so pela hash, unique GLOBAL porque a resolucao acontece antes de haver tenant, revogacao monotonica por trigger), engine_budget_accounts (conta por agente e dia UTC em microusd inteiros, trava da admissao; o budget legado fail-open nao e herdado), engine_inference_attempts (uma linha por request admitido; reserved e o intento persistido antes do provider) e engine_usage_events (eventos idempotentes, append-only, delta NULL = custo desconhecido). Tabelas NOVAS e VAZIAS: sem CONCURRENTLY, envelope BEGIN/COMMIT. Nada referencia a 142/143, entao aplica antes delas
145 | 145_agent_engine_policies.sql | politica de engine por (tenant, agente, canal) para turno novo (spec maia-hermes 4.1, K-15): linha ausente = maia_react, nenhuma flag global liga o Hermes, row_version para CAS, FK composta para channels (tenant_id, agent_id, id) da 090. Tabela NOVA e VAZIA: sem CONCURRENTLY, envelope BEGIN/COMMIT. Down recusa enquanto houver linha hermes. 142/143 estao reservadas por branches locais do P04 e 144 pela PR #768
146 | 146_agent_memories_canonical_link.sql | G2 (spec maia-hermes 7.6.3): agent_memories ganha FK composta para memory_entry (tenant_id, agent_id, id) e content_digest, para o recall autorizado poder exigir item canonico corrente. Coluna nasce NULL porque linhas antigas nao tem como ser ligadas retroativamente; vetor orfao fica ineligivel pelo predicado do recallAuthorized, nao pela coluna. Down derruba FK antes do unique de suporte
147 | 147_agent_canary_policy.sql | escada de canario por (tenant, agente) (spec maia-hermes 10.1, linha P12 do 10): agent_canary_policy guarda o DEGRAU, a coorte autorizada e a evidencia de aceite. A escada em si e codigo (src/runtime/engines/canary-policy.ts) e responde "o que este degrau permite?"; esta tabela responde "em que degrau este agente esta?", que muda por agente e por decisao humana e precisa sobreviver a deploy. Linha ausente = off; nenhuma flag global sobe ninguem de degrau, como a 145 nao tem flag global que ligue o Hermes. Escopo (tenant, agente) e nao (tenant, agente, canal): o 10.1 governa CAPACIDADE, e capacidade nao e do canal — o canal escolhe MOTOR (145), o degrau escolhe o que qualquer motor pode fazer. CHECK exige coorte a partir de live_informational e evidencia de aceite a partir de shadow_offline, duplicando validateCanaryPolicy de proposito: a validacao em TypeScript existe para o console recusar cedo e explicar, a CHECK existe porque o banco e o ultimo lugar por onde toda escrita passa. Sem colunas de percentual/duracao/throughput, que o 10.1 proibe definir sem volume e janela conhecidos. Escrita por CAS em row_version. Tabela NOVA e VAZIA: sem CONCURRENTLY, envelope BEGIN/COMMIT. Down recusa enquanto houver degrau acima de off, porque a evidencia de aceite e justamente o artefato que o 10 exige que exista. 146 esta reservada pela branch do P09 nesta mesma leva
148 | 148_outbound_control_provenance.sql | proveniencia de controle do egresso (spec maia-hermes 8.2.4, C43): outbound_messages ganha origin, control_id e control_epoch. A U-P04.7a fechou o COMMIT (o turno so anda para outbound_pending com a conversa em modo bot) e deixou aberta a janela MAIOR, entre o commit e o envio: a linha espera next_attempt_at e quem a envia e o drain, o recovery ou o takeover de lease, as vezes minutos depois — o operador que assume a conversa nesse intervalo tem a resposta do bot saindo por cima dele. origin separa a fala do BOT da fala do OPERADOR, sem o que o fence reteria justamente quem tem o controle (system fica do lado retido: lembrete automatico durante atendimento humano e a plataforma falando por cima). control_epoch pega pausa seguida de retomada, quando o modo volta a bot e uma resposta escrita ANTES da pausa responderia a um estado que ja nao existe (o backlog que o 8.2.5 descarta em future_only). control_id diz a QUAL controle o epoch pertence, porque epoch e contador por conversa. Colunas nullable: linhas existentes nao tem como ser ligadas retroativamente e conversa nunca pausada nao tem linha de controle; quem retem as legadas e a outra metade do predicado, que olha o controle VIVO da stream do turno. Consumido por outboundEgressAuthorizedSql no WHERE de tryClaimDelivery — ponto UNICO por onde toda entrega do outbox duravel passa, entao o fence e um so e nao tres copias por worker. ADD COLUMN nullable e DEFAULT constante sao metadata-only; CHECKs entram NOT VALID e sao validados em statement proprio, como na 122 e na 135, e por isso o arquivo NAO tem envelope BEGIN/COMMIT (VALIDATE dentro da transacao do ADD varreria a tabela sob ACCESS EXCLUSIVE). Todos os statements idempotentes. 146 esta reservada pela branch do P09 e 147 pela do P12, nesta mesma leva
149 | 149_hermes_runtime_manifests.sql | Immutable scoped synthetic runtime manifests; no production bundle approval
