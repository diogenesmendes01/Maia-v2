...
# O CMD chama sh, que lê o script. O script verifica AUTO_MIGRATE_ON_BOOT
# (default true, fail-closed via gateFlag): se ligado, roda
# `npm run release:migrate` antes de `exec node dist/index.js`. Migration que
# falha impede o app de subir. Compose multi-serviço seta
# AUTO_MIGRATE_ON_BOOT=false no serviço app porque o job migrate separado já
# aplica. Single-container (Coolify) deixa default true.
